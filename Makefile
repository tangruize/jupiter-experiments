...
update:
	...
	-cd $(PROTOCOL_DIR) && for i in `find . -maxdepth 1 -type d | grep Jupiter`; do for j in `ls $$i | grep .tla`; do ln -sf ../../$(TLA_DIR)/$${j} $${i}/ 2>/dev/null; done; done
...
gen-pdf:
	...
	-cd $(PROTOCOL_DIR) && for i in `find . -maxdepth 1 -type d | grep Jupiter`; do for j in `ls $$i | grep .tla`; do ln -sf ../../$(PDF_DIR)/$${j%tla}pdf $${i}/ 2>/dev/null; done; done
